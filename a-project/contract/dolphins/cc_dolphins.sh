...
docker exec cli peer chaincode invoke -n dolphins -C mychannel -c '{"Args":["addDiver","user1","Fish","07-23-1987","F","BO+"]}'
sleep 3

#chaincode invoke add level
# {id, Levelname:args[1], Org:args[2], Instid: args[3]}
docker exec cli peer chaincode invoke -n dolphins -C mychannel -c '{"Args":["addLevel","user1","Advanced","PADI","songjh"]}'
sleep 3

#chaincode invoke add course
# {id, levelname, course}
docker exec cli peer chaincode invoke -n dolphins -C mychannel -c '{"Args":["addCourse","user1","Advanced","Deep diving"]}'
sleep 3

#chaincode invoke add testresult
# {id, levelname, testresult}
docker exec cli peer chaincode invoke -n dolphins -C mychannel -c '{"Args":["addTestResult","user1","Advanced","Qualified"]}'
sleep 3
